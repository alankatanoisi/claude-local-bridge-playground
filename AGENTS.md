...
- Multiple agents (Cursor, Claude Code, Codex) work in this repo, but concurrency means sessions, not edits: only one agent holds the working tree at a time, and an agent starts editing only on a clean, pulled `main` (the tree is the baton — see the four-seat charter bullet below). Keep agent-facing docs and status synchronized across surfaces so every agent starts on the same contextual footing.
...
- Four-seat team charter (2026-08-25, `docs/agent-team-charter-2026-08-25.md`): Alan owns intent and decisions; Fable (Claude Code) explores, builds first slices, and writes execution briefs; Codex executes written briefs and exhaustive tests; Cursor reviews landed work for invariants (with a "not bugs" list). Freeform Alan+Fable sessions stay the default mode; landed runner code gets at least one Cursor invariant review after it lands; the full relay (brief → build → review) is only for safety-boundary or bridge-transport work; the tree is the baton (start only on a clean, pulled `main`).
- After a Cursor invariant review (including `/thermo-nuclear-review`), a chat report is not the handoff. Write a dated agent-facing `HANDOFF-*-thermo-nuclear-*.md` at the repo root: severity-ranked findings, a "not bugs (do not fix)" list, recommended fix order, and pointers for the next agent. Banner the thread-entry handoff so later agents do not treat unverified claims as settled. A clean playground `main` with no feature branch or pull request (a GitHub request to merge one branch into another) is a valid review target — audit the latest landed range; do not invent a pull request. Then **stop**: do not implement, commit, or push unless Alan asks.
