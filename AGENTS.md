...
- Multiple agent sessions (Cursor, Claude Code, Codex) may be open concurrently, but only one agent may write the shared checkout at a time; keep agent-facing docs and status synchronized so every surface starts from the same context.
...
- Four-seat team charter (2026-08-25, `docs/agent-team-charter-2026-08-25.md`): Alan owns intent and decisions; Fable (Claude Code) explores, builds first slices, and writes execution briefs; Codex executes written briefs and exhaustive tests; Cursor reviews landed work for invariants (with a "not bugs" list). Landed runner code needs an independent Cursor invariant review; the full Explore → Brief → Build → Review relay is reserved for safety-boundary or bridge/transport changes. Freeform Alan+Fable sessions stay the default mode; before editing, the one active checkout writer starts from clean, pulled `main` or an explicitly handed-off scoped tree.
