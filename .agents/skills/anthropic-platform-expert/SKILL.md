...
**Do not use this skill** when Alan says `runner-command-builder`, `commands only`, or wants paste-ready `node bin/local-bridge-runner.js` lines — that is **runner-command-builder** (Terminal commands only; no lab-notes edits in Codex).
