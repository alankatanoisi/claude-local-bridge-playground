...
**Concept:** One append-only ledger stores _provider messages_ and _runner state_ (cwd, undo, budgets, allowlist grants) per turn. Resume reads the ledger, not a lossy transcript parser. **Innovation:** Treats continuity as data engineering, not log archaeology. **Risk:** med. **Leverage:** safety, observability. **Spike:** `src/runner/session-ledger.js`, wire in `run.js`.
...
_Generated for parent handoff — ideation only._
